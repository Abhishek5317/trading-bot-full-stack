...
function Backtester({ symbol }) {
  ...
  const handleRunBacktest = () => {
    ...
    fetch(`${API_URL}/api/backtest/${cleanSymbol}`, { method: 'POST' })
      .then(async response => {
      ...
      .catch(error => {
        ...
        setResult({ error: 'Make sure the entered symbol is valid. If it is, please wait for the backend server to wake up.' });
        setLoading(false);
      });
  };
  ...
  const handleStartLive = () => {
    ...
    setLiveStatus(`Starting live trading for ${cleanSymbol}...`);
    fetch(`${API_URL}/api/livetrade/start/${cleanSymbol}`, { method: 'POST' })
      .then(async response => {
        const data = await response.json();
        if (!response.ok) {
  ...
  const handleStopLive = () => {
    ...
    setLiveStatus(`Stopping live trading for ${cleanSymbol}...`);
    fetch(`${API_URL}/api/livetrade/stop/${cleanSymbol}`, { method: 'POST' })
    ...
    <div className="backtester-container">
      <div className="backtester-left">
        <div className="backtester-section">
          ...
          <button onClick={handleRunBacktest} disabled={!cleanSymbol || loading}>
            {loading ? 'Running...' : 'Run Backtest'}
          </button>
        </div>
        <div className="backtester-section">
          <h3>Live Paper Trading</h3>
          <button onClick={handleStartLive} disabled={!cleanSymbol}>Start Live</button>
          <button onClick={handleStopLive} disabled={!cleanSymbol}>Stop Live</button>
